...
@section('content')
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <h1 class="text-3xl font-bold mb-6">{{ __('booking.edit_booking') }}</h1>

                    <!-- Hotel and Room Info -->
                    <div class="bg-gray-50 p-4 rounded-lg mb-6">
                        <h2 class="text-xl font-semibold">{{ $booking->room->hotel->title }}</h2>
                        <p class="text-gray-600">{{ $booking->room->hotel->address }}</p>
                        <div class="mt-2">
                            <h3 class="text-lg font-medium">{{ \App\Helpers\TranslationHelper::translateRoomType($booking->room->type) }}</h3>
                            <p class="text-gray-600">{{ __('room.comfortable_room_description') }}</p>
                            <p class="text-2xl font-bold text-indigo-600 mt-2">
                                {{ \App\Helpers\TranslationHelper::formatPrice($booking->room->price) }} {{ __('booking.per_night') }}
                            </p>
                        </div>
                    </div>
                    ...
                    <form method="POST" action="{{ route('bookings.update', $booking->id) }}">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                            ...
                            <div>
                                ...
                                <select id="children" name="children"
                                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                                    @for($i = 0; $i <= 4; $i++)
                                        <option value="{{ $i }}" {{ old('children', $booking->children) == $i ? 'selected' : '' }}>{{ $i }}</option>
                                    @endfor
                                </select>
                                @error('children')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div class="mt-6 p-4 bg-gray-50 rounded-lg">
                            ...
                            <p class="text-sm text-gray-600 mt-1">
                                <span id="nights-count">{{ $booking->days }}</span> {{ __('booking.nights') }} × <span
                                    id="price-per-night">{{ \App\Helpers\TranslationHelper::formatPrice($booking->room->price) }}</span>
                            </p>
                        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const basePricePerNight = {{ $booking->room->price }};
            ...
            function calculatePrice() {
                ...
                if (startDate && endDate && endDate > startDate) {
                    ...
                    // Format prices according to current locale
                    const locale = '{{ app()->getLocale() }}';
                    const exchangeRates = {
                        'ru': { rate: 1.0, symbol: '₽' },
                        'en': { rate: 0.011, symbol: '$' },
                        'ar': { rate: 0.040, symbol: 'د.إ' },
                        'de': { rate: 0.010, symbol: '€' },
                        'it': { rate: 0.010, symbol: '€' },
                        'fr': { rate: 0.010, symbol: '€' },
                        'es': { rate: 0.010, symbol: '€' }
                    };

                    const currency = exchangeRates[locale] || exchangeRates['ru'];
                    const convertedPricePerNight = Math.round(pricePerNight * currency.rate);
                    const convertedTotalPrice = Math.round(totalPrice * currency.rate);

                    pricePerNightElement.textContent = new Intl.NumberFormat().format(convertedPricePerNight) + ' ' + currency.symbol;
                    totalPriceElement.textContent = new Intl.NumberFormat().format(convertedTotalPrice) + ' ' + currency.symbol;
                }
            }
