<nav x-data="{ open: false }"
    class="bg-white/95 backdrop-blur-md border-b border-white/20 shadow-lg fixed top-0 left-0 right-0 z-50">
        ...
        <div class="flex justify-between h-20">
                            ...
                            <button
                                ...
                                @if(auth()->user() && auth()->user()->avatar)
                                    <img class="h-8 w-8 rounded-full object-cover mr-2"
                                        src="{{ asset('storage/' . auth()->user()->avatar) }}" referrerpolicy="no-referrer" alt="Avatar">
                                @else
            ...
            @else
                <div class="hidden sm:flex sm:items-center sm:ms-6 space-x-4">
                    ...
                    <a href="{{ route('login') }}"
                        class="text-sm text-gray-700 dark:text-gray-500 underline">{{ __('nav.login') }}</a>
                    <a href="{{ route('register') }}"
                        class="ms-4 text-sm text-gray-700 dark:text-gray-500 underline">{{ __('nav.register') }}</a>
                </div>
            @endauth
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        ...
        <div class="pt-4 pb-1 border-t border-gray-200 dark:border-gray-600">
            <div class="px-4">
                <div class="font-medium text-base text-gray-800 dark:text-gray-200">{{ __('nav.language') }}</div>
                <div class="mt-2">
                    <x-language-selector />
                </div>
            </div>
